...
function debug_log($message) {
    error_log("[GET_BOOKING_DEBUG] " . $message);
}
...
try {
    ...
    // Get all bookings
    debug_log("Getting all bookings");
    try {
        $stmt = $db->prepare("
            SELECT 
                b.booking_id,
        ...
        debug_log("Formatted bookings: " . json_encode($formattedBookings));
        echo json_encode($formattedBookings);
        
    } catch (PDOException $e) {
        debug_log("Database error getting bookings: " . $e->getMessage());
        throw $e;
    }
