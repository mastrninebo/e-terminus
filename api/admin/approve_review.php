...
// Debug logging
error_log("=== APPROVE REVIEW DEBUG ===");
...
try {
    ...
    error_log("Processing action: " . $action . " for review_id: " . $review_id);
    
    // Validate review_id
    $stmt = $db->prepare("SELECT review_id FROM reviews WHERE review_id = ?");
    $stmt->execute([$review_id]);
    $review = $stmt->fetch();
    
    if (!$review) {
        error_log("Review not found: " . $review_id);
    ...
    if ($action === 'approve') {
        ...
        $stmt->execute([$review_id]);
        error_log("Review approved: " . $review_id);
        echo json_encode(['success' => true, 'message' => 'Review approved successfully']);
    } else if ($action === 'reject') {
        $stmt = $db->prepare("UPDATE reviews SET is_approved = 0 WHERE review_id = ?");
        $stmt->execute([$review_id]);
        error_log("Review rejected: " . $review_id);
